...
class DishService:
    ...
    async def create_dish(self, submenu_id: UUID, schema: DishRequest) -> Dish:
        ...
        await self._cache_service.set_cache(str(dish.id), dish)
        return dish
    ...
    async def get_dish(self, dish_id: UUID) -> Dish:
        cached_dish = await self._cache_service.get_cache(str(dish_id))
    ...
    async def delete_dish(self, dish_id: UUID):
        await self._cache_service.delete_cache(str(dish_id))
