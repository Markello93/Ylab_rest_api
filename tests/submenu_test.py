...
async def test_get_empty_submenu_list(ac: AsyncClient, menu_id):
...
async def test_patch_submenu(ac: AsyncClient, db_session: AsyncSession, menu_id, submenu_id:UUID4, last_submenu_data):
    data = {
    "title": fake.sentence(),
    "description": fake.sentence()
    }
    ...
    assert response_json["menu_id"] == str(menu_id)


@pytest.mark.run(order=9)
async def test_get_submenu(ac: AsyncClient, db_session: AsyncSession, menu_id, submenu_id:UUID4, last_submenu_data):
    response = await ac.get(f"/api/v1/menus/{menu_id}/submenus/{submenu_id}")
    assert response.status_code == 200

    response_json = response.json()

    assert response_json["id"] == str(submenu_id)
    assert response_json["title"] == last_submenu_data["title"]
    assert response_json["description"] == last_submenu_data["description"]


@pytest.mark.run(order=10)
async def test_get_submenu_list(ac: AsyncClient, menu_id):
    response = await ac.get(f"/api/v1/menus/{menu_id}/submenus/")
    assert response.status_code == 200
    response_json = response.json()
    assert response_json != []
...
async def test_del_submenu(ac: AsyncClient, db_session: AsyncSession, menu_id:UUID4, submenu_id:UUID4):
