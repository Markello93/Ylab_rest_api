...
async def test_get_empty_dishes_list(ac: AsyncClient, test_ids):
    response = await ac.get(f"/api/v1/menus/{test_ids['menu_id']}/submenus/{test_ids['submenu_id']}/dishes/")
...
async def test_create_dish(ac: AsyncClient, db_session: AsyncSession, test_ids, last_submenu_data):
    data = {
        "title": fake.sentence(),
        "description": fake.sentence(),
        "price": str(fake.pydecimal(left_digits=3, right_digits=2, positive=True))
    }
    response = await ac.post(f"/api/v1/menus/{test_ids['menu_id']}/submenus/{test_ids['submenu_id']}/dishes/", json=data)
...
async def test_patch_dish(ac: AsyncClient, db_session: AsyncSession, test_ids, last_dish_data):
    data = {
    "title": fake.sentence(),
    "description": fake.sentence(),
    "price": str(fake.pydecimal(left_digits=3, right_digits=2, positive=True))
    }
    response = await ac.patch(f"/api/v1/menus/{test_ids['menu_id']}/submenus/{test_ids['submenu_id']}/dishes/{test_ids['dish_id']}", json=data)
...
async def test_get_dish(ac: AsyncClient, db_session: AsyncSession,last_dish_data, test_ids):
...
async def test_del_dish(ac: AsyncClient, db_session: AsyncSession, test_ids):
    response = await ac.delete(f"/api/v1/menus/{test_ids['menu_id']}/submenus/{test_ids['submenu_id']}/dishes/{test_ids['dish_id']}")
    assert response.status_code == 200, f"Expected status code 200, got {response.status_code} instead"
